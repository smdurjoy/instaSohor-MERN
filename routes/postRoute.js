...
router.get('/', auth, async (req, res) => {
    try {
        const posts = await Post.find({ postedBy: req.user }).sort('-createdAt').populate('postedBy', '_id name username').populate('comments.commentedBy', '_id name username')
...
router.get('/all', auth, async (req, res) => {
    try {
        const posts = await Post.find().sort('-createdAt').populate('postedBy', '_id name username').populate('comments.commentedBy', '_id name username')
        return res.status(200).json(posts)
    } catch(err) {
        res.status(500).json({ error: err.message })
    }
...
router.get('/following-posts', auth, async (req, res) => {
    try {
        ...
        if(followingArray.following.length < 1)
            return res.status(201).json({"msg": "You are not following anyone !"})

        const followingPosts = await Post.find({postedBy: {$in:followingArray.following}}).sort('-createdAt').populate('postedBy', '_id name username').populate('comments.commentedBy', '_id name username')
...
router.get('/:username', auth, async (req, res) => {
    try {
        ...
        const posts = await Post.find({ postedBy: user }).sort('-createdAt').populate('postedBy', '_id name username').populate('comments.commentedBy', '_id name username')
        return res.status(200).json(posts)
    } catch(err) {
        res.status(500).json({ error: err.message })
    }
